...

{
  ...
  systemd.services.post-uptime-webhook = {
      ...
      script = ''
        awk '{print "{\"content\":\"HexF Uptime: "int($1/86400)":"int(($1%86400)/3600)":"int(($1%3600)/60)":"int($1%60)"\"}"}' /proc/uptime | curl -XPOST -H "Content-Type: application/json" --data @- $(cat ${config.sops.secrets.snowflake_uptime_webhook.path})
      '';
  };
